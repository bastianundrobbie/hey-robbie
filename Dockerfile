...
#
# Base MUST be glibc: the Anthropic SDK ships a native helper binary that is
# dynamically linked (needs glibc) → Debian slim, NOT Alpine/musl.
# Runs as non-root uid 1000: that helper refuses to run as root.
...
# Python deps first (layer cache). The SDK wheel is large (~230 MB, it
# bundles a native helper). --chown on the FIRST copy: it creates
